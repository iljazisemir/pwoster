...
import "../../styles/components/Post/RepwostPost.css";
import { useDispatch } from "react-redux";
import { repwostPost, unrepwostPost } from "../../actions/post.actions";
...
const RepwostPost = ({ className, post, userPost }) => {
  ...
  const retweet = (e) => {
    e.preventDefault();
    dispatch(repwostPost(post._id, uid));
    setRetweeted(true);
  ...
  const unretweet = (e) => {
    e.preventDefault();
    dispatch(unrepwostPost(post._id, uid));
    setRetweeted(false);
    setPopupRetweet(false);
  };
